    ...
    <div className="modal-overlay" onClick={onClose} style={{
      ...
      <div className="modal-content glass-effect" onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '20px', overflowY: 'auto', flex: 1 }}>
          ...
          {/* Processes Table */}
          <h3 style={{ fontSize: '1rem', marginBottom: '15px', color: 'var(--text-color)' }}>Processi (Ordinati per CPU)</h3>
                ...
                <tbody>
                  {processes.slice(0, 50).map(p => (
                    <tr key={p.pid} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <td style={{ padding: '10px', color: 'var(--primary)' }}>{p.cpu.toFixed(1)}%</td>
                      <td style={{ padding: '10px' }}>{p.mem.toFixed(1)}%</td>
